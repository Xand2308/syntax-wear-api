...
Status de pedido: Enum ("PENDING", "PAID", "SHIPPED", "DELIVERED", "CANCELLED") para garantir consistência
Cancelamento de pedido: NÃO implementar reversão de estoque (sem rollback)
Webhook de pagamento: NÃO implementar endpoint de confirmação de pagamento
...
Se produto tem sizes, validar que size foi informado e está disponível
CEP no formato correto (8 dígitos)
Items não vazio, quantities >= 1
Guest Checkout
...
⏳ Schema + Migration (Order, OrderItem, enum OrderStatus)
⏳ Service (orders.service.ts com transações Prisma)
⏳ Validators + Types (Zod schemas e interfaces TypeScript)
